...
use crate::channel::{Channel, TOKEN_LINK, channel_id_for_token};
use crate::protocol::{
    F_ACK, F_DATA, F_HELLO, F_PING, F_PONG, F_TDATA, FrameParser, LINK_HIGH_WATER, LINK_LOW_WATER,
    TxQueue, build_frame,
};
...
impl Daemon {
    ...
    fn open_link(&mut self, poll: &Poll) {
        ...
        match open_serial_fd(&dev, 115200) {
            Ok(fd) => {
                if let Err(e) =
                    poll.registry()
                        .register(&mut SourceFd(&fd), TOKEN_LINK, Interest::READABLE)
                {
    ...
    fn close_link(&mut self, reason: &str, poll: &Poll) {
        ...
        if self.link_bp {
            ...
            for ch in channels.values_mut() {
                ch.resume_source_reads(reg);
            }
        }
        let channels = &mut self.channels;
        let reg = poll.registry();
        for ch in channels.values_mut() {
            ch.on_link_disconnect(reg);
        }
        self.schedule_reopen();
    }
    ...
    fn enqueue_frame(&mut self, frame: Vec<u8>, poll: &Poll) {
        if self.disconnected || self.link_fd.is_none() {
            return;
        }
            ...
            F_HELLO => {
                ...
                if self.link_up {
                    ...
                    for id in ch_ids {
                        ...
                        let reg = poll.registry();
                        if let Some(c) = ch.get_mut(&id) {
                            c.on_link_connect(txq, reg);
                        }
                    }
                    self.last_tx = Some(Instant::now());
    ...
    fn next_timeout(&self, now: Instant) -> Duration {
        ...
        if self.disconnected {
            ...
            if let Some(lt) = self.last_tx {
                deadline = deadline.min(lt + KA_INTERVAL);
            }
            if let Some(lr) = self.last_rx {
                deadline = deadline.min(lr + KA_TIMEOUT);
            }
        }
        ...
        loop {
            ...
            for event in events.iter() {
                ...
                if token == TOKEN_LINK {
                    ...
                    if writable && self.link_fd.is_some() {
                        self.link_write(poll);
                    }
